...
// Approved Programs
class ApprovedPrograms extends React.Component {
  ...
  async componentDidMount() {
    ...
    httpGet(apify(`app/approved-programs`)).then(res => {
      if (res['success']) {
        this.setState({
                  ...
                  <div className="card-body p-0">
                    {
                      <DataTable
                        ...
                        noDataComponent={<DatatableNoRows text="There are no programs Approved." />}
                        pagination
                      />
                    }
                  </div>
...
//
export default withRouter(ApprovedPrograms);