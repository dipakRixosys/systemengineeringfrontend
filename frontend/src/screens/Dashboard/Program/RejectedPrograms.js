...
// Rejected Programs
class RejectedPrograms extends React.Component {
  ...
  async componentDidMount() {
    ...
    httpGet(apify(`app/rejected-programs`)).then(res => {
      if (res['success']) {
        this.setState({
                  ...
                  <div className="card-body p-0">
                    {
                      <DataTable
                        ...
                        noDataComponent={<DatatableNoRows text="There are no programs Rejected." />}
                        pagination
                      />
                    }
                  </div>
...
//
export default withRouter(RejectedPrograms);